...
#' @examples
#'
#' @references [1] Ehsani R, Drabløs F: \strong{TopoICSim: a new semantic
...
topo_ic_sim_g1g2 <- compiler::cmpfun(function(gene1,
    ...
    for (i in 1:m) {
        for (j in 1:n) {
            if (go1[i] == go2[j])
                ...
                # now check if they are present, if 0 rows are returned, then
                ...
                if (nrow(DF) != 0) {
                  ...
                  # if this is not the case (row is not present), then run
                  # topo_ic_sim between 2 terms.
    ...
    if (!sum(!is.na(scores)))
        return(list(geneSim = NA, GO1 = go1, GO2 = go2))
    scores <- sqrt(scores)  # Some scores seem unnecesary to calculate? Like,
    # when they have the same GOID the similarity will always be 1.
    ## YES, I ADDED A NEW LINE FOR THIS CASE ABOVE //if(gene1==gene2)
