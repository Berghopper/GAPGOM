...
Prepare GO data for random data selection and topoICSim term calculation.
...
```
```{r, fig.width=6, fig.height=3}
final_times <- data.frame(GAPGOM:::benchmarks$server_times_genset, GAPGOM:::benchmarks$laptop_times_genset, seq_along(GAPGOM:::benchmarks$laptop_times_genset))
colnames(final_times) <- c("server", "laptop", "n")
final_times_long <- melt(final_times, id="n")
ggplot(data=final_times_long,
       aes(x=n, y=value, colour=variable)) +
    geom_point() + stat_smooth(method = "lm", formula = y ~ x + I(x^2), size = 1)
```


As you can see, TopoICSim does not scale well 
