...
#expdata <- ExpressionData[,c(1,3,4:ncol(ExpressionData))]

#apply(expdata[3:ncol(expdata)],1,function(x) {all(x==0)})

colnames(expdata)[1] <- "ENSEMBL"
...
# set gid and run of Pyruvate Carboxylase.
gid <- expdata[expdata[[2]] == "PC",][[1]]
...
result <- GAPGOM::expression_prediction(gid, 
                                        ...
                                        "MF",
                                        method = "combine", 
                                        verbose = TRUE, go_amount = 5,
                                        significance = 0.05, enrichment_cutoff = 250, idtype = "ENSEMBL")
go_annotation_prediction <- unique(as.vector(result$GOID))

geneset_ensembl_topvar <- names(rev(sort(apply(expression_matrix[geneset_ensembl,], 1, var)))[1:5])
geneset_filtered <- expdata[expdata$ENSEMBL %in% geneset_ensembl_topvar,]$V2
geneset_filtered <- geneset_filtered[geneset_filtered!="PC"]
orig <- GAPGOM::topo_ic_sim_genes("human", "MF", "PC", geneset_filtered, custom_genes1 = NULL, idtype = "SYMBOL")

cus <- GAPGOM::topo_ic_sim_genes("human", "MF", c(), geneset_filtered, custom_genes1 = list(PC=go_annotation_prediction), idtype = "SYMBOL")
```
