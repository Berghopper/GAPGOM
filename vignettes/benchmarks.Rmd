...
for (ontology in ontologies) {
  ...
  tmp_go_data <- set_go_data("human", ontology, computeIC = T, 
                             keytype = "SYMBOL")
  bench_results[[ontology]] <- mapply(
    function(gos, name) {
      ...
      return(GAPGOM::topo_ic_sim_genes("human", ontology, c(), name_orig, 
                                   ...
                                   idtype = "SYMBOL", 
                                   progress_bar = F, go_data = tmp_go_data)$GeneSim)
...
for (ontology in ontologies) {
  ...
  brplot <- ggplot(dat, aes(x=Gene, weight=sim)) + geom_bar() + coord_flip() +
    labs(title = paste(strwrap("Similarity of custom gene versus original", 
                               width = 30), collapse = "\n")) +
    theme(plot.title = element_text(hjust = 0.5)) +
