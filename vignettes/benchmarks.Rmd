...
#geneset <- read.table("~/Downloads/geneset(1).txt", sep="\n") # APOPTOSIS
...
entrezidsample <- sample(expdata[[1]], 1000)
#entrezidsample_filtered <- expdata[ apply(expdata[expdata[[1]] %in% entrezidsample,3:ncol(expdata)],1,mean) > 25,][[1]]
fakefilter <- unique(c(entrezidsample, entrezidsample)) #sample(geneset_ensembl, 15)))
# set gid and run.
#gid <- "AGRN"
gid <- sample(geneset_ensembl, 1)
fakefilter <- fakefilter[!(fakefilter == gid)]
...
#GAPGOM::expression_semantic_scoring(gid, expset)
```
