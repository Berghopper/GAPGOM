...
```{r expset, eval=F}
library(Matrix)
expression_matrix <- Matrix(as.matrix(expressions[,2:ncol(expressions)]))
...
ExpressionData = read.table('./lncRNA2function_data.txt', sep='\t', head=TRUE)

# Gene ID's and annotation term type.
EnsemblID2GOID = read.table('./EG2GO.txt', sep='\t',head=TRUE)
